...
onebitdata = np.zeros(data.shape, dtype=np.uint8)
from runningmean import runningmean
import ctypes
runningmean(ctypes.c_void_p(data.ctypes.data), ctypes.c_int(nsamp), ctypes.c_int(nchan), ctypes.c_void_p(dataout.ctypes.data))
